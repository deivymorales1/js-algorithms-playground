...
function getMaxGifts(giftsCities, maxGifts, maxCities) {
  if (!giftsCities >= 1 && !maxGifts >= 1 && !maxCities >= 1) {
    return 0;
  }
  ...
  function generateCombinations(
    ...
    if (currentSum > maxGifts || citiesCount > maxCities) {
      return;
    }

    if (currentSum > maxGiftsSum) {
      maxGiftsSum = currentSum;
    }

    for (let i = start; i < giftsCities.length; i++) {
      const giftsInCity = giftsCities[i];
      currentCombination.push(giftsInCity);
      generateCombinations(
        i + 1,
        currentCombination,
        currentSum + giftsInCity,
        citiesCount + 1
      );
...
const result = getMaxGifts([50, 70, 30], 100, 4);
console.log(result);
